...
class NestedSetBehavior extends CActiveRecordBehavior
{
    ...
    protected function tree2array($nodes) {
        $res = array();
        foreach($nodes as $n) {
            $item = array('label'=>$n->title, 'url'=>$n->link);

            if ($n->access == Menu::ACCESS_AUTHENTICATED) {
                $item['visible'] = !Yii::app()->user->isGuest;
            }

            if ($n->access == Menu::ACCESS_ADMINS) {
                $item['visible'] = Yii::app()->getModule('user')->isAdmin();
            }
            if(!$n->isLeaf()) {
    ...
    protected function categories2array($nodes) {
        $res = array();
        foreach($nodes as $n) {
            $item = array('label'=>$n->title, 'url'=>$n->url);
            $res[] = $item;
        }
        return $res;
    }
    ...
    public function getCategoriesArray() {
        return $this->categories2array($this->ancestors()->findAll());
    }
